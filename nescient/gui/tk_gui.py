...
from nescient.timing import benchmarks, estimate_time, TkTimer, benchmark_mode
    ...
    def update_rate_info(self):
        ...
        for mode in self.modes:
            if mode in benchmarks:
                sizes = benchmarks[mode]
                largest = sorted(sizes.keys())[-1]
                duration = sizes[largest]
                rate = round(largest / 2**20 / duration, 1)
